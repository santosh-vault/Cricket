...
function ICCRankingsCard() {
  ...
  return (
    <div className="bg-white rounded-2xl p-6 shadow-md flex flex-col text-gray-900 border border-gray-100">
  ...
  return (
    <>
      ...
      {!fixturesLoading && !fixturesError && (
        <section className="py-8 bg-gradient-to-r from-blue-50 to-indigo-100 border-b border-blue-200 shadow-inner">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <FixturesList
              ...
              className="bg-white rounded-xl shadow-md p-4 md:p-6" // Apply styles to the FixturesList wrapper
            />
          </div>
        </section>
      )}
              ...
              {latestNews.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                  ...
                  <Link key={latestNews[0].id} to={`/news/${latestNews[0].slug}`} className="block md:col-span-2 group">
                    <article className="bg-white rounded-2xl shadow-md hover:shadow-md transition-all duration-300 overflow-hidden flex flex-col h-full border border-gray-100">
                  ...
                  <div className="flex flex-col gap-8">
                    {latestNews.slice(1, 4).map((article) => (
                      <Link key={article.id} to={`/news/${article.slug}`} className="block group">
                        <article className="bg-white rounded-2xl shadow-md hover:shadow-md transition-shadow duration-300 overflow-hidden flex flex-col md:flex-row h-full border border-gray-100">
                          {article.thumbnail_url ? (
                            <div className="w-full h-40 md:w-40 md:h-auto flex-shrink-0 bg-blue-100 flex items-center justify-center overflow-hidden">
                              <img
                            ...
                            <div className="w-full h-40 md:w-40 md:h-auto flex-shrink-0 bg-blue-100 flex items-center justify-center p-4">
                              <TrendingUp className="h-10 w-10 text-blue-400 opacity-70" />
                            </div>
                          )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {featuredBlogs.length > 0 ? featuredBlogs.map(blog => (
                <Link key={blog.id} to={`/blogs/${blog.slug}`} className="group">
                  <article className="bg-white rounded-2xl shadow-md hover:shadow-md transition-all duration-300 overflow-hidden flex flex-col md:flex-row h-full border border-gray-100">
                    {blog.thumbnail_url ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {featurePosts.length > 0 ? featurePosts.map(post => (
              <Link key={post.id} to={`/features/${post.slug}`}
                className="bg-white rounded-xl shadow-md hover:shadow-md transition-shadow overflow-hidden group flex flex-col items-stretch w-full max-w-xs mx-auto min-h-[370px]"
              >
                {post.thumbnail_url ? (
