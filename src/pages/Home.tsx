...
function ICCRankingsCard() {
  const [selectedFormat, setSelectedFormat] = useState<"test" | "odi" | "t20">(
    "test"
  );
  ...
  async function fetchRankings() {
    ...
    if (error) {
      ...
      setRankings(data || []);
      // Get the most recent update time from the rankings
      ...
      <div className="mb-5">
        ...
        {lastUpdated && (
          <div className="text-xs text-gray-500 flex items-center mt-2">
            ...
            Updated {format(new Date(lastUpdated), "MMM dd, yyyy")}
          </div>
        )}
      </div>
        ...
        {loading ? (
          ...
          <>
            {rankings.slice(0, 7).map((row) => (
              <div
                key={row.id}
                className="flex items-center py-3 text-base justify-between group"
              >
                ...
                <span className="flex-1 text-gray-800 font-medium text-lg group-hover:text-blue-700 transition-colors duration-200">
                  {row.team_name}
                </span>
            ...
            {rankings.length > 7 && (
              <Link
                to="/ranking"
                className="block w-full mt-2 py-2 text-center bg-blue-50 text-blue-700 font-semibold rounded-lg hover:bg-blue-100 transition-colors duration-200 show-all-rankings flex items-center justify-center"
              >
...
export const Home: React.FC = () => {
  ...
  const renderImageOrPlaceholder = (
    ...
    return thumbnail_url ? (
      ...
      <div
        className={`${placeholderClass} bg-gray-100 flex flex-col items-center justify-center text-blue-400`}
      >
                          ...
                          <div className="p-6 flex flex-col flex-1 justify-between">
                            ...
                            <div className="flex justify-between items-center text-base text-gray-500 mt-auto">
                              <span className="flex items-center">
                                ...
                                {format(
                                  new Date(section1[0].created_at),
                                  "MMM dd, yyyy"
                                )}
                              </span>
                    ...
                    <div className={`flex flex-col h-full gap-6`}>
                      {section1.slice(1).map((item, idx) => (
                        <Link
                          ...
                          className="block group h-1/2 flex-1"
                        >
                          <article
                            className={`bg-white rounded-2xl transition-all duration-300 overflow-hidden flex flex-col h-full border border-gray-100 ${smallCardMinH}`}
                          >
                            ...
                            <div className="p-4 flex-1 flex flex-col justify-between">
                              ...
                              <div className="flex items-center text-xs text-gray-500 mt-auto">
                                ...
                                <span className="ml-auto text-blue-600 font-semibold flex items-center read-more group-hover:text-blue-800">
                                  Read More{" "}
                                  <ChevronRight className="h-3 w-3 ml-1 transform group-hover:translate-x-1 transition-transform duration-200" />
                                </span>
                              </div>
                            </div>
                          </article>
