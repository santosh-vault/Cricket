...
export const Ranking: React.FC = () => {
  ...
  const [allRankings, setAllRankings] = useState<
    Record<"test" | "odi" | "t20", RankingItem[]>
  >({
    test: [],
    odi: [],
    t20: [],
  ...
  return (
    <>
      <SEOHead
        title="ICC Cricket Rankings - Cricket Fantom"
            ...
            {loading ? (
              <div className="text-center py-20 text-gray-400">
                ...
                <p className="text-xl font-medium">
                  Loading all formats for {selectedCategory} rankings...
                </p>
              </div>
            ) : (
                ...
                <div className="flex-grow divide-y divide-gray-100">
                  {allRankings[selectedFormat].length > 0 ? (
                    allRankings[selectedFormat].map((row: RankingItem) => (
                      <div
                        key={row.id}
                        className="flex items-center py-3 text-base justify-between hover:bg-blue-100 rounded-md px-2 -mx-2 transition-colors duration-200"
                      >
                        <span className="w-8 text-gray-700 font-bold text-lg">
                          {row.rank}.
                        </span>
                        {(() => {
                          ...
                          if (
                            row.flag_emoji &&
                            typeof row.flag_emoji === "string" &&
                            row.flag_emoji.length > 0
                          ) {
                            return (
                              <ReactCountryFlag
                        ...
                        <span className="flex-1 text-gray-900 font-semibold truncate mr-2">
                          {row.team_name}
                        </span>
                    ...
                    <div className="text-center text-gray-500 py-10 text-md">
